...
class BloxorzSuite extends FunSuite {
  ...
  test("neighbors with history") {
    new Level1 {
      ...
      assert(expected === neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)).toSet)
    }
  }
  ...
  test("new neighbors only") {
    new Level1 {
      ...
      val allNeighbors = Set(
        (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ).toStream

      assert(expected === newNeighborsOnly(allNeighbors, explored).toSet)
    }
  }
