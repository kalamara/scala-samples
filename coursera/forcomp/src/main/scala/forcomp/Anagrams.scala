...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case Nil => Nil
    case h::t => wordOccurrences(s.reduce(_ + _))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    val stretched = for(k <- occurrences) yield stretch(k._1, k._2)

    val self = selfRelate(stretched)

    val r = for(s <- self) yield correlate(s._1, s._2)
   ...
   */
  def remove(occurrences: Occurrences, ch: Char, freq: Int) = {
    val occ = occurrences.toMap

    val fr = occ.getOrElse(ch, 0)
    val newfr = if( freq < fr ) fr - freq else 0

    val result = if (fr > 0)
      if( newfr > 0)
        occ.updated(ch, newfr)
      else occ - ch
    else occ
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match {
    case Nil => x
    case h::t => subtract(remove(x, h._1, h._2), t)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    val words = dictionaryByOccurrences
    def iter(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
      else for {
        combination <- combinations( occurrences )
        word <- words getOrElse (combination, Nil)
